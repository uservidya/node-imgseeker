...
var _ensure = function ($, selector, index, property) {
  ...
  return $selected[index][property] || $selected.css('background-image').replace(/url\(|\)|"|'/g, '');
},
...
ImgSeeker.prototype = {
  ...
  response: function(err, result, $, config, cb) {
    var img;
    if (!$ && !err) { err = 'cannot access the page'; }
    ...
    if (config[host]) {
      ...
      img = (function () {
        ...
        for (i = 0; i < config[host].length; i++) {
          _sel = config[host][i].split('|');
          _img = _ensure($, _sel[0], _sel[1] || 0, 'src');
          if (_img) {
            return _img;
          }
        }
